...
task.spawn(function()
    while true do
        pcall(function()
            if yo()  >= 8e6 and yo()  < 30e6 and lplr.Status.Transformation.Value == "None" then
                Ex.equipskill:InvokeServer("Godly SSJ2")
                Ex.ta:InvokeServer()
            elseif yo()  >= 30e6 and lplr.Status.Transformation.Value == "Godly SSJ2" then
                Ex.ta:InvokeServer()
            end
        end)
        task.wait()
